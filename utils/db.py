...
# Ensure the parent directory exists; fall back to /tmp on read-only filesystems
_db_dir = os.path.dirname(os.path.abspath(DB_PATH))
try:
    os.makedirs(_db_dir, exist_ok=True)
except OSError:
    DB_PATH = "/tmp/cardshop.db"
...
def add_transaction(
    ...
    with get_conn() as conn:
        conn.execute(
            """INSERT INTO transactions (user_id, order_id, type, description, amount, status)
               VALUES (?, ?, ?, ?, ?, ?)""",
